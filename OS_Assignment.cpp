...
void CheckSafe( Vect2D need, Vect2D allocation, vector<int> work)
{
	vector<int> Sequence;
	int count = 0;
...
void Request( int Process , vector<int> REQ ,  vector<int> available, Vect2D need, Vect2D allocation)
{
	for(int col=0; col<NumOfResources ; col++)
	{
	...
	CheckSafe( need, allocation , available );
}
...
int main()
{
	...
	if (option==1){
		...
		for(int i=0; i<NumOfResources; i++)
			cin>>Req[i];
		Request(Process, Req, Available , Need, Allocation);
	}
